...
function generateUUID(): string {
  return (([1e7] as any) + -1e3 + -4e3 + -8e3 + -1e11).replace(
    /[018]/g,
    (c: any) => (c ^ (crypto.randomBytes(1)[0] & (15 >> (c / 4)))).toString(16),
  );
}
...
if (argv.userId || argv.email || argv.companyId || argv.roles) {
  ...
  const requesterToken = jwt.sign(requesterPayload, privateKey, {
    algorithm: "RS256",
  });
  console.error("\nRequester JWT Token:");
  ...
  const responderToken = jwt.sign(responderPayload, privateKey, {
    algorithm: "RS256",
  });
  console.error("\nResponder JWT Token:");
